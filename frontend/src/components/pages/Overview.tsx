...
// Radar kesehatan: `norm` DITURUNKAN dari ambang alarm KPI yang SAMA
// (statusOf) supaya SELALU sinkron — mustahil radar bilang "sehat" saat KPI
// alarm merah. good→0.5..1 (skala dalam pita aman), warning→0.35, critical→0.12.
function healthMetrics(kpi: Record<string, number>): HexMetric[] {
  // fraksi 0..1 di dalam pita aman (untuk gradasi halus, bukan cuma good/warn)
  const inBand = (v: number, lo: number, hi: number, invert = false) => {
    const t = Math.max(0, Math.min(1, (v - lo) / (hi - lo)));
    return invert ? 1 - t : t;
  };
  const rows: Array<[string, string, number, number]> = [
    ["Recovery", "recovery_pct", kpi.recovery_pct, inBand(kpi.recovery_pct, 85, 97)],
    ["Efisiensi Biaya", "total_opex", kpi.total_opex, inBand(kpi.total_opex, 15000, 25000, true)],
    ["Silika Bersih", "reactive_sio2_pct", kpi.reactive_sio2_pct, inBand(kpi.reactive_sio2_pct, 2, 5.5, true)],
    ["Minim Red Mud", "red_mud_t", kpi.red_mud_t, inBand(kpi.red_mud_t, 300, 500, true)],
    ["Potensi CO₂", "co2_capture_t", kpi.co2_capture_t, inBand(kpi.co2_capture_t, 5, 16)],
    ["Yield", "precip_yield_pct", kpi.precip_yield_pct, inBand(kpi.precip_yield_pct, 79, 82)],
  ];
  return rows.map(([label, key, v, frac]) => {
    // ambang alarm memutuskan zona; frac memberi gradasi di dalam zona
    const st: Severity = statusOf(key, v);
    const norm = st === "good" ? 0.55 + 0.45 * frac
      : st === "warning" ? 0.35 : 0.12;
    return { label, value: norm, norm, grade: grade(norm) };
  });
}
